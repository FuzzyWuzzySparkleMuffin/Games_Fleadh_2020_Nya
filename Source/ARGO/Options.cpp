...
void Options::render(SDL_Renderer* t_renderer)
{
	...
	SDL_RenderCopy(t_renderer, cactusTexture, NULL, &cactusRect);
	SDL_RenderCopy(t_renderer, stateTexture, NULL, &stateRect);
	SDL_RenderCopy(t_renderer, levelCompleteTexture, NULL, &levelCompleteRect);
...
void Options::loadSprites(SDL_Renderer* renderer)
{
	if (m_backgroundTexture == NULL)
	{
		...
		cactusSurface = SDL_LoadBMP("ASSETS/IMAGES/CactusHurtBlack.bmp");
		cactusTexture = SDL_CreateTextureFromSurface(renderer, cactusSurface);
		stateSurface = SDL_LoadBMP("ASSETS/IMAGES/SwapStatesBlack.bmp");
		stateTexture = SDL_CreateTextureFromSurface(renderer, stateSurface);
		levelCompleteSurface = SDL_LoadBMP("ASSETS/IMAGES/LevelCompleteBlack.bmp");
		levelCompleteTexture = SDL_CreateTextureFromSurface(renderer, levelCompleteSurface);
