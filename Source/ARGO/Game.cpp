...
GameState Game::m_currentMode{ GameState::licence};
...
void Game::init(const char* title, int xpos, int ypos, int width, int height, bool fullscreen)
{
	...
	m_characterVectorArray.push_back(m_factory->initEntityCactus(cactus, Vector2(tempMap.getCactusPos()), Vector2(60, 60), "ASSETS/IMAGES/cactus.bmp", true, "ASSETS/AUDIO/removedCactus.wav", false, *m_renderer));
	...
	//cactus.getComponent<AudioComponent>().loadAudioCactusFeedback("ASSETS/AUDIO/removedCactus.wav");
	platform.getComponent<AudioComponent>().loadAudioPaperRip("ASSETS/AUDIO/paperRip.wav");
	newPlayer.getComponent<AudioComponent>().loadAudioInputUI("ASSETS/AUDIO/UIuse.wav");
...
{
	if (!SDL_JoystickGetHat(stick.getStick(), 0) == SDL_HAT_LEFT ||
		!SDL_JoystickGetHat(stick.getStick(), 0) == SDL_HAT_RIGHT)
	{
		dpadPlayed = false;
	}
	...
	case GameState::mainMenu:
		...
		if (SDL_JoystickGetHat(stick.getStick(), 0) == SDL_HAT_DOWN ||
			SDL_JoystickGetHat(stick.getStick(), 0) == SDL_HAT_UP) 
		{
			if (m_count > 20)
			{
				newPlayer.getComponent<AudioComponent>().playAudioInputUI();
				m_count = 0;
			}
...
void Game::update()
{
	...
	//	cactus.getComponent< AudioComponent>().playAudioPaperRip();
		
		cactus.getComponent<AudioComponent>().playAudioCactusFeedback();
