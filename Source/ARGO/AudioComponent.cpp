...
void AudioComponent::loadMusicFileLevel(const char* t_string)
{
	...
	Mix_VolumeChunk(m_levelChunk, 10);
...
void AudioComponent::loadAudioPaperRip(const char* t_string)
{
	m_paperRip = Mix_LoadWAV(t_string);
	if (m_paperRip == nullptr)
	{
		std::cout << "error loading file in audio:" << t_string << std::endl;
	}
}

void AudioComponent::loadAudioCactusFeedback(const char* t_string)
{
	m_cactusFeedback = Mix_LoadWAV(t_string);
	if (m_cactusFeedback == nullptr)
	{
		std::cout << "error loading file in audio:" << t_string << std::endl;
	}
}
...
void AudioComponent::playAudioPaperRip()
{
	Mix_PlayChannel(-1, m_paperRip, 0);
}

void AudioComponent::playAudioCactusFeedback()
{
	Mix_PlayChannel(-1, m_cactusFeedback, 0);
}
